...
const NowPlayingMovies = ({ data, page }) => {
  ...
  return (
    <section className="py-[1rem] px-[1.5rem] md:px-[3rem] lg:py-[1.5rem] lg:max-w-[85rem] w-full">
      ...
      <Heading isHomePage={false} title2="Now Playing" />
      ...
      <div className="flex justify-center mt-[2.25rem]">
        <Pagination
          prevPath={`/movie/now-playing/${currentPage - 1}`}
          nextPath={`/movie/now-playing/${currentPage + 1}`}
...
export const getServerSideProps = async (context) => {
  ...
  const res = await fetch(
    `https://api.themoviedb.org/3/movie/now_playing?api_key=${process.env.NEXT_PUBLIC_TMDB_API_KEY}&page=${slug}`
  );
